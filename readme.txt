Overview
This project is a very lean implementation of a Kubernetes JMeter cluster. It will allow for the execution of a JMeter test script on an arbitrarily sized JMeter test rig and then will generate:

A JMeter DashBoard Report
JMeter Test Log (JTL)
To provide a simple method of deleting the test rig after the test execution we are using the Kubernetes Namespace feature which allows for an expedited method to delete everything created.

kubectl delete namespace NameSpacdName
...
Execute the following PowerShell CreateTestRig.ps1
...
File Name: jmeter_cluster_create.ps1 This will create 1 JMeter Master pod and 2 or more JMeter Slave pods. It also creates the K8S master configurtion map and the JMeter Slaves service. Once the script is completed you can check the status of the Pods with the following kubectl command:
...
To delete tbe cluster you run the following PowerShell DeleteTestRig.ps1
