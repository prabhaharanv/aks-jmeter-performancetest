...
#az ad sp create-for-rbac --skip-assignment --name aks-jm
log "Creating aks cluster"
#create a service principal in azure by executing 'az ad sp create-for-rbac --skip-assignment --name aks-jm' in Windows Powershell and replace ${appId}, ${password} with values for "appId" and "password" respectively in next line
az aks create --resource-group $AksResourceGroup   --name $AksClusterName   --node-count 1 --enable-addons monitoring --generate-ssh-keys  --service-principal ${appId} --client-secret ${password} --enable-cluster-autoscaler --min-count 1 --max-count 3
